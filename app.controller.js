function ChatBotController($http, $window, $scope) {
  ...
  function generateResponse(incomingChatLi) {
    ...
    $http
      .post(API_URL, requestOptions.body, requestOptions.headers)
    ...
    try {
      ...
      if (threadInLocalStorage) {
        ...
        const response = await $http.delete(
          `https://lms.kz/gpt/api/delete-history?threadId=${threadId}`
        );
  ...
  $window.document
    ...
    .addEventListener("input", ctrl.handleInputChange);
  $window.document
    .querySelector(".chat-input textarea")
    .addEventListener("keydown", ctrl.handleKeyDown);
    ...
    try {
      ...
      if (threadInLocalStorage) {
        ...
        var response = await $http.get(
          `https://lms.kz/gpt/api/get-history?threadId=${threadId}`
        );
        ...
        var createThreadResponse = await $http.post(
          "https://lms.kz/gpt/api/create-thread",
          { username: user }
        );
        if (createThreadResponse.status === 200) {
          ...
          console.error(
            ...
            createThreadResponse.statusText
          );
          setTimeout(function () {
            ctrl.isLoading = false;
            $scope.$apply();
...
angular.module("myModule").component("chatBot", {
